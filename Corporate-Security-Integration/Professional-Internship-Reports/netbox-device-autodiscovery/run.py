...
def create_or_update_nb_obj(obj_type: str, obj: dict) -> Optional[int]:
    ...
    If object does not exist or does not match, it will be created or updated.:param obj_type: NetBox object type, must match keys in self.obj_map
    ...
    if existing_nb_obj:
        if obj_type in NETBOX_OBJECTS_DELETION_ORDER:
            if 'Autodiscovered' not in (tag.name for tag in existing_nb_obj.tags):
                log.debug(
                    'NetBox %s object "%s" skipped because has no "Autodiscovered" tag.',
        ...
        if compare_nb_objects(obj, prepare_pynetbox_result(existing_nb_obj.__dict__)):
            ...
            # ✅ VÉRIFICATION POUR LES DEVICES : erreur bloquante si pas de rôle
            if obj_type == 'devices' and 'role' not in obj:
                log.error(f"Device '{obj.get('name', 'unknown')}' is missing the required 'role'.")
                return None
        ...
        # ✅ ATTRIBUTION AUTOMATIQUE D'UN RÔLE SI MANQUANT (création)
        if obj_type == 'devices' and 'role' not in obj:
            log.warning(f"Device '{obj.get('name', 'unknown')}' has no 'role' defined. Assigning default role 'Other'.")
            obj['role'] = {'name': 'Other'}
...
def main():
    ...
    for module_name in config['data_sources']['modules']:
        ...
        if module_name == 'avaya_ip_office_phones_enumerator':
            log.info('Module "avaya_ip_office_phones_enumerator" désactivé temporairement. On passe.')
            continue  # Ignore ce module et passe au suivant

        log.info(f'Running module "{module_name}"…')
        module_config = config[module_name]
        if module_config is None:
            log.warning(f'Module "{module_name}" désactivé temporairement. On passe.')
            continue
